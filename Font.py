...
class Font:
    ...
    def variant(self, string: str, bold: bool = False, darken: bool = False, italic: bool = False, underline: bool = False, flashing: bool = False, reverse_display: bool = False, blanking: bool = False, reset: bool = True) -> str:
        '''
        处理字符串变体
        '''
        ...
        if bold == True:               # 高亮
            variant = f"\033[{self.Variant.BOLD}m"
        elif darken == True:              # 低亮
            variant = f"\033[{self.Variant.DARKEN}m"
        elif italic == True:                # 斜体
    ...
    def __init__(self, string: str = None, Fore: str = "", Back: str = None, bold: bool = False, darken: bool = False, italic: bool = False, underline: bool = False, flashing: bool = False, reverse_display: bool = False, blanking: bool = False, reset: bool = True) -> None:
        '''
        构造函数
        '''
        ...
        if bold == True:               # 高亮
            ansi = f"\033[{self.Variant.BOLD};"
            if Fore != "":           # 不为空
                if Back == None:            # 无背景
                    ansi += f"{Fore}m"
                else:                       # 有背景
                    ansi = f"\033[{self.Variant.BOLD};{Back}m\033[{self.Variant.BOLD};{Fore}m"
            else:                           # 为空
                ansi = f"\033[{self.Variant.BOLD};{Back}m"
        elif darken == True:               # 低亮
            ansi = f"\033[{self.Variant.DARKEN};"
            if Fore != "":           # 不为空
                if Back == None:            # 无背景
                    ansi += f"{Fore}m"
                else:                       # 有背景
                    ansi = f"\033[{self.Variant.DARKEN};{Back}m\033[{self.Variant.DARKEN};{Fore}m"
            else:                           # 为空
                ansi = f"\033[{self.Variant.DARKEN};{Back}m"
        elif italic == True:               # 斜体
